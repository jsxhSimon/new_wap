...
//    NSString* IsScreenStr = [command argumentAtIndex:3];
    
    
    CDVThemeableBrowserOptions* browserOptions = [self parseOptions:options];

    BOOL IsScreen = browserOptions.browerfullscreen;
    ...
    if (url != nil) {
        ...
        if ([target isEqualToString:kThemeableBrowserTargetSelf]) {
            [self openInCordovaWebView:absoluteUrl withOptions:options withScreen:IsScreen];
        } else if ([target isEqualToString:kThemeableBrowserTargetSystem]) {
            [self openInSystem:absoluteUrl];
        } else { // _blank or anything else
            [self openInThemeableBrowser:absoluteUrl withOptions:options withScreen:IsScreen];
        }
...
- (void)openInThemeableBrowser:(NSURL*)url withOptions:(NSString*)options withScreen:(BOOL)isScreen

{
    ...
    if (self.themeableBrowserViewController == nil) {
        self.themeableBrowserViewController = [[CDVThemeableBrowserViewController alloc]
                                               ...
                                               settings:self.commandDelegate.settings withScreen:isScreen];
...
- (void)openInCordovaWebView:(NSURL*)url withOptions:(NSString*)options withScreen:(BOOL)isScreen
{
...
- (id)initWithBrowserOptions:(CDVThemeableBrowserOptions*) browserOptions navigationDelete:(CDVThemeableBrowser*) navigationDelegate statusBarStyle:(UIStatusBarStyle) statusBarStyle settings:(NSDictionary*) settings withScreen:(BOOL)isScreen
{
    self = [super init];
    if (self != nil) {
...
- (void)createViews
{
    ...
    csCloseBtnBounds.origin.x = insets.left + 15;
...
//    self.progressView=[[UIProgressView   alloc] initWithFrame:CGRectMake(0.0, toolbarY+toolbarHeight+[self getStatusBarOffset], self.view.bounds.size.width, 20.0)];
    ...
    if([_browserOptions.orientation isEqualToString:@"LANDSCAPE"]){
        if (self.IsScreen) {
            self.loadingWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_HEIGHT+insets.top*2, SCREEN_WIDTH + insets.bottom)];
        }else{
            self.loadingWebView = [[WKWebView alloc] initWithFrame:CGRectMake(insets.top, 0, SCREEN_HEIGHT, SCREEN_WIDTH + insets.bottom)];
        }
      
    }else {
        self.loadingWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH,SCREEN_HEIGHT + insets.bottom+insets.top)];
    }
...
//    NSLog(@"%@", NSStringFromCGPoint(translation));
    ...
    {
        availableWidth -= (insets.left + 15);
    }else { //按钮在右半屏
        availableWidth -= (insets.right + 15);
    }
...
- (void) rePositionViews {
    ...
    if (self.IsScreen && isIphonex) {
        
        if ([_browserOptions.orientation isEqualToString:@"LANDSCAPE"]) {
            [self.webView setFrame:CGRectMake(0, insets.top, SCREEN_WIDTH+ 94 ,SCREEN_HEIGHT - insets.top - insets.bottom)];
        }else{
            [self.webView setFrame:CGRectMake(insets.left, 0, SCREEN_WIDTH - insets.left - insets.right,SCREEN_HEIGHT+94)];
        }
    }else{
