...
export const getDownloadUrl = () => {
  const { APP_DOWNLOAD_LDY: ldy, APP_DOWNLOAD_URL_IOS: ios, APP_DOWNLOAD_URL_ANDROID: and } = process.env
  return `${JSON.parse(ldy ?? '""')}?ios=${encodeURIComponent((JSON.parse(ios ?? '[]')).join(','))}&and=${encodeURIComponent(JSON.parse(and ?? '[]'))}&spreadCode=${window.location.host}`
}

// 获取北京时间时间戳
export const getTimeStamp = () => {
  const currentDate = new Date(new Date().getTime().toString() + SessionStorage.getItem('timeGap') + ((parseInt((new Date().getTimezoneOffset() / 60).toString(), 10) + 8) * 3600 * 1000))
